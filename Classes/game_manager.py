...
#     def __init__(self):
#         pg.init()
#         self.main_surface = pg.display.set_mode((s.SCREEN_WIDTH, s.SCREEN_HEIGHT))
#         pg.display.set_caption("Flappy Bird AI")
#         self.clock = pg.time.Clock()
        
#         # Initialize scoreboard window
#         self.scoreboard_surface = pg.Surface((s.SCOREBOARD_WIDTH, s.SCOREBOARD_HEIGHT))
...
#         self.generation_scores.append(best_fitness)
...
#         # Clear scoreboard with a gradient background
#         for y in range(s.SCOREBOARD_HEIGHT):
#             r = int(200 - (y / s.SCOREBOARD_HEIGHT) * 100)
#             g = int(200 - (y / s.SCOREBOARD_HEIGHT) * 100)
#             color = (r, g, 230)
#             pg.draw.line(self.scoreboard_surface, color, (0, y), (s.SCOREBOARD_WIDTH, y))
        
#         # Title
#         title = self.title_font.render("Generation Scores", True, (255, 255, 255))
#         title_rect = title.get_rect(centerx=s.SCOREBOARD_WIDTH//2, top=20)
#         self.scoreboard_surface.blit(title, title_rect)
        
#         # Render scores with modern look
#         for i, score in enumerate(reversed(self.generation_scores[-10:]), 1):
#             # Alternate row colors for better readability
#             row_color = (240, 240, 240) if i % 2 == 0 else (220, 220, 220)
#             row_rect = pg.Rect(20, 80 + i * 40, s.SCOREBOARD_WIDTH - 40, 35)
#             pg.draw.rect(self.scoreboard_surface, row_color, row_rect, border_radius=10)
            
#             # Generation and score text
#             gen_text = self.score_font.render(f"Gen {len(self.generation_scores)-i+1}", True, (0, 0, 0))
#             score_text = self.score_font.render(f"Best Fitness: {score:.2f}", True, (0, 0, 50))
            
#             self.scoreboard_surface.blit(gen_text, (40, 85 + i * 40))
#             self.scoreboard_surface.blit(score_text, (s.SCOREBOARD_WIDTH // 2, 85 + i * 40))
